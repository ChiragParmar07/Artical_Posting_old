...
export const getArticles = createAsyncThunk('articles/getArticles', async (user, { rejectWithValue }) => {
    try {
...
export const getArticlesByTopic = createAsyncThunk('articles/getArticlesByTopic', async (topicId, { rejectWithValue }) => {
    try {
        const userData = JSON.parse(localStorage.getItem('userData'))
        const token = userData.token
...
export const getArticlesById = createAsyncThunk('articles/getArticlesById', async (articleId, { rejectWithValue }) => {
    try {
        const userData = JSON.parse(localStorage.getItem('userData'))
        const token = userData.token
...
export const addArticle = createAsyncThunk('articles/addArticle', async (article, { rejectWithValue }) => {
    try {
        const userData = JSON.parse(localStorage.getItem('userData'))
        const token = userData.token
        const response = await axios.post('http://127.0.0.1:5000/api/article', article, {
            headers: {
                'authorization': `Bearer ${token}`
            }
        })
...
export const updateArticle = createAsyncThunk('articles/updateArticle', async (data, { rejectWithValue }) => {
    try {
        const userData = JSON.parse(localStorage.getItem('userData'))
        const token = userData.token
