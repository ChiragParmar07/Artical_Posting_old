...
export const getTopics = createAsyncThunk('topics/getTopics', async (user, { rejectWithValue }) => {
    try {
...
export const addTopic = createAsyncThunk('topics/AddTopic', async (topic, { rejectWithValue }) => {
    try {
        const userData = JSON.parse(localStorage.getItem('userData'))
        const token = userData.token
        const response = await axios.post('http://127.0.0.1:5000/api/topic', topic, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        })
